...
def center_focal_loss(
    ...
    float_logits = logits.to(dtype=torch.float32)
    float_targets = targets.to(dtype=torch.float32)
    probabilities = float_logits.sigmoid().clamp(min=1e-6, max=1.0 - 1e-6)
    positive_mask = float_targets.eq(1.0).to(float_logits.dtype)
    negative_mask = float_targets.lt(1.0).to(float_logits.dtype)
